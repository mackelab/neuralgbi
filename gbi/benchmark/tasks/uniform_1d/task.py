...
class UniformNoise1D:
    ...
    def simulate_noiseless(self, theta: Tensor) -> Tensor:
        """Noiseless simulator."""
        return (torch.hstack([(.8*(theta+0.25))**i for i in range(5)]) * self.poly_coeffs).sum(1, keepdim=True)
    
    def simulate(self, theta: Tensor) -> Tensor:
        """Simulator with U[-eps, eps] noise applied."""
        # Get uniform noise of [-epsilon, epsilon].
        noise = self.noise_likelihood.sample((theta.shape[0],))
    ...
    def distance_fn(self, theta):
        ...
        expected_value_of_d = self._eval_distance_integral(x_high) - self._eval_distance_integral(x_low)
