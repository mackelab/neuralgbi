...
def compute_predictives(theta_samples, task, distance_fn_empirical, inference_obj=None):
    ...
    if distance_fn_empirical==mmd_dist:        
        predictives["dist_gt"] = torch.Tensor([task.distance_fn(th) for th in theta_samples])
    else:
    ...
    if inference_obj:
        with torch.no_grad():
            ...
            # ).squeeze(1)
            predictives['dist_estimate'] = inference_obj.predict_distance(theta_samples, task.x_o[None,:])
    else:
